...
class Laser(Sensor):
	...
	def get_observation(self, data):
		laser_vector = np.frombuffer(data, dtype=np.float32).reshape(self.__y_rays, self.__x_rays)
		if self.__show:
			self.__show_laser_on_screen(laser_vector)
		return laser_vector

	def change_settings(self, start_angle_x=None, end_angle_x=None, distance_angle_x=None, start_angle_y=None, end_angle_y=None, distance_angle_y=None, laser_range=None, Render=None):
		...
		if Render is not None:
			self.self.__settings_sensor['Render'] = Render
