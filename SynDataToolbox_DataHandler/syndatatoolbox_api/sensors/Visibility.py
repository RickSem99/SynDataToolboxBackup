...
class Visibility(Sensor):
	...
	def __init__(self,name, buffer_size,stringifyed_setup_dict:str, params_from_environment:dict=None):
		super().__init__()
		self.__name = name
		self.__buffer_size = buffer_size
		self.__command = 'OBS_{}'.format(self.__name)
		self.__settings_sensor = self.parse_dict(stringifyed_setup_dict)
		self.__show = False
	...
	def get_observation(self, data):
		sensors = np.frombuffer(data, dtype=np.float32).reshape(self.__x_rays, -1, 2)
		laser_vector1 = sensors[:, :int(sensors.shape[1]/3)]
		laser_vector2 = sensors[:, int(sensors.shape[1]/3):int(sensors.shape[1]/3*2)]
		laser_vector3 = sensors[:, int(sensors.shape[1]/3*2):]

		if self.__show:
			self.__show_laser_on_screen(laser_vector1)

		return [laser_vector1, laser_vector2, laser_vector3]

	def change_settings(self, start_angle_x=None, end_angle_x=None, distance_angle_x=None, start_angle_y=None, end_angle_y=None, distance_angle_y=None, laser_range=None, cells_range=None, Render=None):
		...
		if cells_range is not None:
			self.__settings_sensor['CellsRange'] = cells_range
