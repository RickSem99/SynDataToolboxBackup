...
#include "Components/Sensors/GPSSDT.h"
...
{
	if (!TargetToTrack) {
		UE_LOG(LogTemp, Error, TEXT("UNSET ACTOR TO TRACK"))
		return false;
	}

	StartLocation = TargetToTrack->GetActorLocation();
	StartRotation = TargetToTrack->GetActorRotation();
...
const bool AGPSSDT::TakeObs()
{
	...
	bIsBusy = true;
	FRotator CurrentAbsoluteRotation = TargetToTrack->GetActorRotation();
	FVector CurrentAbsoluteLocation = TargetToTrack->GetActorLocation();

	FRotator CurrentRelativeRotation = CurrentAbsoluteRotation - StartRotation;
	FRotator Rotation = StartRotation.GetInverse();
	FVector  CurrentRelativeLocation = Rotation.RotateVector(CurrentAbsoluteLocation - StartLocation);

	uint8* FloatNumber = new uint8[sizeof(float)];

	LastObservationTimestamp = FDateTime::UtcNow().ToString();
	float f = CurrentAbsoluteLocation.X;
	char* s = (char*)&f;
	memcpy(FloatNumber, s, sizeof(f));
	...
	f = CurrentAbsoluteRotation.Pitch;
	s = (char*)&f;
	memcpy(FloatNumber, s, sizeof(f));
	...
	f = CurrentAbsoluteRotation.Roll;
	s = (char*)&f;
	memcpy(FloatNumber, s, sizeof(f));
	...
	f = CurrentRelativeRotation.Pitch;
	s = (char*)&f;
	memcpy(FloatNumber, s, sizeof(f));
	...
	f = CurrentRelativeRotation.Roll;
	s = (char*)&f;
	memcpy(FloatNumber, s, sizeof(f));
