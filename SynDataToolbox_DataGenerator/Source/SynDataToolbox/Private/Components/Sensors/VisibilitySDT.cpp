...
#include "Components/Sensors/VisibilitySDT.h"

// Sets default values
AVisibilitySDT::AVisibilitySDT()
{
...
void AVisibilitySDT::BeginPlay()
{
	Super::BeginPlay();
	if (TimeSampling != 0.0f) {
		SetTickMode(false);
		FTimerHandle ObservationTimerHandle;
		GetWorld()->GetTimerManager().SetTimer(ObservationTimerHandle, this, &AVisibilitySDT::TakePeriodicObs, TimeSampling, true,0.0f);
	}
...
// Called every frame
void AVisibilitySDT::Tick(float DeltaTime)
{
...
void AVisibilitySDT::TakePeriodicObs()
{
	TakeObs();
}

const uint32 AVisibilitySDT::GetObservationSize()
{
...
const bool AVisibilitySDT::InitSensor()
{
...
const FString AVisibilitySDT::GetSensorSetup()
{
...
const bool AVisibilitySDT::ChangeSensorSettings(const TArray<FString>& Settings)
{
	while (bIsBusy) {}
	if (Settings.Num() == 10)
	{
...
const bool AVisibilitySDT::GetLastObs(uint8* Buffer)
{
	while (bIsBusy) {}
	if (LastObservation) {
		bIsBusy = true;
...
const FString AVisibilitySDT::GetSensorName()
{
	return "Visibility(" + GetActorLabel() + ")";
}

const void AVisibilitySDT::SetTickMode(bool Value)
{
	enableIndependentTick = Value;
	PrimaryActorTick.bCanEverTick = enableIndependentTick;
}

const bool AVisibilitySDT::TakeObs()
{
