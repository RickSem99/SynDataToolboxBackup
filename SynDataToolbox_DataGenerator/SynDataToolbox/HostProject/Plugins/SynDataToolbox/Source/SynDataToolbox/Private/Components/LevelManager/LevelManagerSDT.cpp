#include "Components/LevelManager/LevelManagerSDT.h"
...
void ULevelManagerSDT::AppendNewActor(AActor* NewActor)
{
	if (!ActorList.IsEmpty()) 
	{
		...
		if (PaintableComponents.Num() > 0 && !NewActor->GetName().Contains("SDT")) 
		{
...
void ULevelManagerSDT::FillActorList()
{
	...
	for (TActorIterator<AActor> ActorIt(WorldPtr); ActorIt; ++ActorIt)
	{
		...
		if (!PaintableComponents.IsEmpty() && !ClassName.Contains("APIGateway")) {
			if (ClassName.Contains("SDT") || ClassName.Contains("Pawn")){
