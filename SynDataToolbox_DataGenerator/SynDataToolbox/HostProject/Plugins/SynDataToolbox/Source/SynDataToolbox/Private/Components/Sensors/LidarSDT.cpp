...
#include "Components/Sensors/LidarSDT.h"
#include <string>
...
void ALidarSDT::BeginPlay()
{
	Super::BeginPlay();
	if (TimeSampling != 0.0f) {
		SetTickMode(false);
		FTimerHandle ObservationTimerHandle;
		GetWorld()->GetTimerManager().SetTimer(ObservationTimerHandle, this, &ALidarSDT::TakePeriodicObs, TimeSampling, true,0.0f);
	}

	InitSensor();
}

void ALidarSDT::TakePeriodicObs()
{
	TakeObs();
}

// Called every frame
void ALidarSDT::Tick(float DeltaTime)
{
...
const FString ALidarSDT::GetSensorSetup() {
...
const bool ALidarSDT::ChangeSensorSettings(const TArray<FString>& Settings)
{
	if (Settings.Num() == 8)
	{
		bIsBusy = true;
...
const bool ALidarSDT::GetLastObs(uint8* Buffer)
{
	while (bIsBusy) {}
	if (LastObservation) {
		if (Modified) {
...
const bool ALidarSDT::TakeObs()
{
	...
	for (float Y = StartAngleY; Y <= EndAngleY; Y += DistanceAngleY)
	{
		for (float X = StartAngleX; X <= EndAngleX; X += DistanceAngleX)
		{
			...
			bIsBusy = true;
			float fX = ImpactLocation.X;
			char* sX = (char*)&fX;
			memcpy(FloatNumber, sX, sizeof(fX));
			...
			float fY = ImpactLocation.Y;
			char* sY = (char*)&fY;
			memcpy(FloatNumber, sY, sizeof(fY));
			LastObservation[Index] = FloatNumber[0];
			Index++;
			LastObservation[Index] = FloatNumber[1];
			Index++;
			...
			float fZ = ImpactLocation.Z;
			char* sZ = (char*)&fZ;
			memcpy(FloatNumber, sZ, sizeof(fZ));
			LastObservation[Index] = FloatNumber[0];
			Index++;
			LastObservation[Index] = FloatNumber[1];
			Index++;
...
const FString ALidarSDT::GetSensorName()
{
	return "Lidar("+GetActorLabel() + ")";
}

const void ALidarSDT::SetTickMode(bool Value)
{
