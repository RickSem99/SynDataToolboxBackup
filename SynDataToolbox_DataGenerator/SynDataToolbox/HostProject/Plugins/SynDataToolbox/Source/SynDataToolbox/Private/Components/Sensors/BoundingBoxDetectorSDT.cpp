#include "Components/Sensors/BoundingBoxDetectorSDT.h"

ABoundingBoxDetectorSDT::ABoundingBoxDetectorSDT()
{
	PrimaryActorTick.bCanEverTick = enableIndependentTick;
	Camera = CreateDefaultSubobject<USceneCaptureComponent2D>(TEXT("BoundingBoxDetectorSDT"));
...
void ABoundingBoxDetectorSDT::BeginPlay()
{
	Super::BeginPlay();
	...
	if (!LevelManager) {
		...
		for (auto& Pair : ClassesToBound) {
			if (!Pair.Key.Equals("Others")) {
				BoundingBoxes.Add(Pair.Value, FBoundingBoxCollectionSDT());
			}
		}
	}
	...
	if (TimeSampling != 0.0f) {
		...
		GetWorld()->GetTimerManager().SetTimer(ObservationTimerHandle, this, &ABoundingBoxDetectorSDT::TakePeriodicObs, TimeSampling, true, 0.0f);
	}
}

void ABoundingBoxDetectorSDT::TakePeriodicObs()
{
	TakeObs();
}

FString ABoundingBoxDetectorSDT::ClassesMapToString()
{
...
void ABoundingBoxDetectorSDT::Tick(float DeltaTime)
{
...
const uint32 ABoundingBoxDetectorSDT::GetObservationSize()
{
	return uint32(MaxActors*10);
}

const bool ABoundingBoxDetectorSDT::InitSensor()
{
...
const FString ABoundingBoxDetectorSDT::GetSensorSetup()
{
	...
	return "{Width:" + FString::FromInt(Width) + "," +
		...
		"Classes:{" + ABoundingBoxDetectorSDT::ClassesMapToString() + "}" +
		"}";
}

const bool ABoundingBoxDetectorSDT::ChangeSensorSettings(const TArray<FString>& Settings)
{
	if (Settings.Num() == 4)
	{
...
const bool ABoundingBoxDetectorSDT::GetLastObs(uint8* Buffer)
{
...
const bool ABoundingBoxDetectorSDT::TakeObs()
{
...
const FString ABoundingBoxDetectorSDT::GetSensorName()
{
	return "BoundingBox("+GetActorLabel()+")";
}

const void ABoundingBoxDetectorSDT::SetTickMode(bool Value)
{
	enableIndependentTick = Value;
	PrimaryActorTick.bCanEverTick = enableIndependentTick;
}

TMap<FString, int> ABoundingBoxDetectorSDT::ParseDictFromString(FString Str)
{
